...
This functions are copied form the dcurves package:
- https://github.com/MSKCC-Epi-Bio/dcurves
The code has been edited so we can generate multiple subplots in the same figure and edit the subplots
axes. Thank Chatgpt hehe
...
def _plot_net_benefit(
    ...
    ax : matplotlib.axes.Axes, optional
        The axis on which to plot. If None, uses plt.gca().
    ...
    for idx, modelname in enumerate(modelnames):
        ...
        if smoothed_data and modelname in smoothed_data:
            ...
            ax.plot(smoothed[:, 0], smoothed[:, 1], color=color, label=modelname)
        else:
            ax.plot(model_df["threshold"], model_df["net_benefit"], color=color, label=modelname)
...
def my_plot_graphs(
    ...
    ax : matplotlib.axes.Axes, optional
        The axis on which to plot. If None, the plotting function uses plt.gca() and displays the figure directly.
    ...
    # Choose the correct plotting function and pass the axis
    plot_function = (
    ...
    if file_name:
        try:
            if ax is None:
                ...
                ax.get_figure().savefig(file_name, dpi=dpi)
        except Exception as e:
            print(f"Error saving figure: {e}")
    # Only show the plot if no axis is provided (i.e. standalone usage)
    if ax is None:
        plt.show()
