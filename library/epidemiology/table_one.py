...
class MakeTableOne:
    ...
    def create_table(self, show:Optional[bool]=False) -> pd.DataFrame:
    ...
    def _categorical_var_dist(
        ...
        if category is not None:
            ...
            for level, count in counts.items():
                ...
                results.append(f"{level}: {count}({percent}%)")
            return results
    ...
    @staticmethod
    def group_variables_table(df: pd.DataFrame, show:Optional[bool]=False) -> pd.DataFrame:
        """
        ...
        if show:
            print(tabulate(df_transformed,
                           headers=[*df_transformed.columns],
                           showindex=False,
                           tablefmt="fancy_grid"))
