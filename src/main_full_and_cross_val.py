...
from library.plot_functions import (plot_model_metrics, plot_model_metrics_specific_columns,
                                    plot_elastic_net_model_coefficients,
...
if __name__ == '__main__':
    ...
    OVERWRITE = True
    PLOT_VEN = False
    base_path = config.get('results_path').get('full_cross_val')
    ...
    configurations = {
        ...
        'Questionnaire Only': {
            'features': [col for col in columns if not col in ['age', 'gender', target]],
            'target': target,
        },
    ...
    if not (path_avg_metrics_config.is_file() and path_classifications_config.is_file()) or OVERWRITE:
        ...
        for conf_name, conf_values in configurations.items():
            ...
            # 21 FULL‐DATA (no cross‐validation) training
            (
                df_full_metrics,
            ...
            df_all_classifications = pd.concat([df_all_classifications, df_classifications_fold], ignore_index=True)
    ...
    # sort the frames
    df_all_metrics_fold = df_all_metrics_fold.sort_values(by=['model', 'config', 'specificity'],
